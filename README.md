...
```bash
battle-pythons$ sam build --use-container
```
...
```bash
battle-pythons$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
battle-pythons$ sam local start-api
battle-pythons$ curl http://localhost:3000/
```
...
```bash
battle-pythons$ sam logs -n HelloWorldFunction --stack-name battle-pythons --tail
```
...
```bash
battle-pythons$ pip install -r tests/requirements.txt --user
# unit test
battle-pythons$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
battle-pythons$ AWS_SAM_STACK_NAME="battle-pythons" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "battle-pythons"
```
